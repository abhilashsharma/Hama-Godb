...
public class BFSDistr extends
      SubgraphComputeWrapup<BFSDistrSubgraphState, MapWritable, MapWritable, Text, LongWritable, LongWritable, LongWritable> {
...
//							System.out.println("*************getOutEdges***********:" + count);
			...
			for(VertexMessageSteps stuff: getSubgraph().getSubgraphValue().forwardRemoteVertexList){
				...
				IRemoteVertex<MapWritable,MapWritable,LongWritable,LongWritable,LongWritable> remoteVertex = (IRemoteVertex)getSubgraph().getVertexById(new LongWritable(stuff.vertexId));//remoteVertex=null if remote inedge
