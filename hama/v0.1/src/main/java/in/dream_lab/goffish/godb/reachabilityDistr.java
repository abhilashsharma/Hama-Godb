...
import in.dream_lab.goffish.SubgraphComputeWrapup;
...
SubgraphComputeWrapup<reachabilityDistrSubgraphState, MapWritable, MapWritable, Text, LongWritable, LongWritable, LongWritable> {
...
//	}
	...
	@Override
	public void compute(Collection<IMessage<LongWritable,Text>> _messageList) {
		
		Collection<IMessage<LongWritable,Text>> messageList = _messageList;
...
//				System.out.println("***********DONE**************");
          ...
          for (IVertex<MapWritable, MapWritable, LongWritable, LongWritable> sourceVertex : getSubgraph()
              .getLocalVertices())
            for (IEdge<MapWritable, LongWritable, LongWritable> edge : sourceVertex
                .getOutEdges()) {
              ...
              IVertex<MapWritable, MapWritable, LongWritable, LongWritable> sinkVertex = getSubgraph()
...
//							System.out.println("COMPARING:" + getSubgraph().getSubgraphValue().endVertex.value.toString() + " To " + otherVertex.getValue().get(new Text(getSubgraph().getSubgraphValue().endVertex.property)).toString());
							if ( !(compareValuesUtil(otherVertex.getValue().get(new Text(getSubgraph().getSubgraphValue().endVertex.property)).toString(), getSubgraph().getSubgraphValue().endVertex.value.toString())) ){
...
//							System.out.println("Remote Vertex found");
							IRemoteVertex<MapWritable,MapWritable,LongWritable,LongWritable,LongWritable> remoteVertex = (IRemoteVertex<MapWritable, MapWritable, LongWritable, LongWritable, LongWritable>)otherVertex;
...
//					System.out.println("ANY Inedges +"+subgraph.getId()+":" + InEdges.containsKey(currentVertex.getId()) + " Contains:" + subgraph.containsVertex(currentVertex.getId()));
					...
					for(Map.Entry<Long, EdgeAttr> edgeMap: getSubgraph().getSubgraphValue().InEdges.get(currentVertex.getVertexId().get()).entrySet()) {
						...
						IVertex<MapWritable,MapWritable,LongWritable,LongWritable> otherVertex = getSubgraph().getVertexById(new LongWritable(otherVertexId));
						...
						if ( !edgeMap.getValue().isRemote ) {
							...
							if (!compareValuesUtil(otherVertex.getValue().get(new Text(getSubgraph().getSubgraphValue().startVertex.property)).toString(), getSubgraph().getSubgraphValue().startVertex.value.toString()) ){
								if (vertexMessageStep.stepsTraversed<getSubgraph().getSubgraphValue().noOfSteps){
